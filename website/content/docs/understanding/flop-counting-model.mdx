...
## FMA convention: textbook (multiply + add = 2 operations)

flopscope counts a fused multiply-add (`a * b + c`) as **2 operations** —
one multiply + one add, counted separately. This matches the textbook /
opt_einsum convention. Under this model, dense matmul of `(m, k)` × `(k, n)`
costs `2·m·k·n − m·n` FLOPs (m·n output cells, each doing k multiplies and
k−1 adds with the accumulator initialized from the first product).

There is no longer a configurable knob — the previous `flops.fma_cost()` /
`flops.configure(fma_cost=...)` API was removed in the 2026-05-20
unification. flopscope is FMA=2-only.
...
instructions to analytical FLOPs (FMA = 2 ops, textbook), measured via
...
```python
adjusted_cost = analytical_cost × weight(op_name)
```

Symmetry reductions and weights apply independently -- symmetry reduces
the element count, the weight scales the per-element cost.
