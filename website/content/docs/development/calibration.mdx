...
Symmetry reductions and weights apply independently -- symmetry reduces the element count, the weight scales the per-element cost.
...
- in **perf** mode, it is the median ratio of counter-observed retired instructions to the analytical FLOP count (FMA = 2 ops, textbook)
...
- **weight < 1.0** -- the operation is cheaper per analytical FLOP than `np.add` under the chosen calibration mode. Under flopscope's FMA=2 textbook convention, BLAS/linalg ops measured against `fp_arith_inst_retired` (which also counts FMAs as 2 ops) should give weights near 1.0; deviations reflect kernel efficiency or benchmarking noise.
...
Full per-operation weight data is available in the [API Reference](/docs/api/) and on each standalone operation page under the canonical `/docs/api/...` routes, for example `/docs/api/einsum/`.
