...
- ``FLOPSCOPE_DISABLE_WEIGHTS=1`` disables all weights.
- ``FLOPSCOPE_WEIGHTS_FILE=/path/to/file.json`` loads an explicit override.
...
def _load_packaged_weights() -> dict[str, float] | None:
    try:
        ...
        _warn_once(
            "Flopscope client could not load packaged official weights "
...
def load_weights(path: str | None = None, *, use_packaged_default: bool = True) -> None:
    ...
    if override_path is not None:
        try:
            ...
            _warn_once(
                f"Flopscope client could not load custom weights from '{override_path}' "
